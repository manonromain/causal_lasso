...
def dyn_no_lips(X, W0, dagness_exp, dagness_pen, l1_pen, verbose=False, logging=False, eps=1e-4, max_iter=500, mosek=True):
    """ Main algorithm """
    ...
    if logging:
        logging = {"dagness_exp": dagness_exp, "dagness_pen": dagness_pen, "l1_pen": l1_pen,  # constants
                   "time": [], "ll_trace": [], "l1_val": [], "log_det": [], "dag_constraint": [],
    ...
    while (it_nolips < 2 or (np.abs((nnl_prev - nnl_curr)/nnl_prev) >= eps)) and it_nolips < max_iter:
        ...
        if logging:
            support = Wk > 0.5
            logging["time"].append(time.time() - start)
            logging["ll_trace"].append(nnl_curr)
            logging["dag_constraint"].append(dag_penalty_k/dagness_pen)
            logging["l1_val"].append(np.sum(Wk))
            logging["nb_change_support"].append(np.sum(prev_support ^ support))
            logging["support"].append(support.flatten())
    ...
    if logging:
        logging_np = {k: np.array(v) for k, v in logging.items()}
